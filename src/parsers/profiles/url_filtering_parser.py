...
class URLFilteringParser(BaseParser):
    ...
    def _parse_section(self, section: ET.Element, source_type: str) -> List[Dict]:
        """Parse URL Filtering profiles from a specific section."""
        profiles = []
        ...
        try:
            ...
            for entry in entries:
                try:
                    ...
                    profile_data = {
                        ...
                        "default_action": {
                            "action": default_action_element.findtext("action", "allow")
                        },
            ...
            self.logger.info(
                f"Parsing successful for {len(profiles)} URL Filtering profiles from '{source_type}' section"
            )
            return profiles

        except Exception as e:
            self.logger.error(
                f"Error parsing '{source_type}' URL Filtering profiles section: {str(e)}"
            )
            return profiles
