...
class DOSRuleTransformer(BaseTransformer):
    ...
    def _create_match_section(self, data: Dict[str, Any], logger: Any) -> Dict[str, Any]:
        """Create match section of Versa DOS rule."""

        match: Dict[str, Any] = {
            "source": {"address": {}},  # Ensure this is a dictionary
            "destination": {
                "zone": {"zone-list": data.get("to", [])},  # List inside a nested dictionary
                "address": {},  # Ensure this is a dictionary
            },
            "ip-version": "ipv4",  # String
            "schedule": "",  # String
            "services": {"predefined-services-list": []},  # Fix: Make this a dictionary instead of a list
        }
        ...
        match["services"]["predefined-services-list"] = services  # Keep services as a dictionary
